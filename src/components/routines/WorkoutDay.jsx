...
import { Activity, Dumbbell, Flame, Zap, Clock, CheckCircle, Circle, ChevronDown, Info, Edit } from 'lucide-react';
...
const WorkoutDay = ({
  ...
  return (
    <>
      {/* Routine Header Card */}
      <div className={`p-5 rounded-2xl border mb-6 ${routine.bg} ${routine.border} relative overflow-hidden`}>
                  ...
                  <div key={i} className={`transition-colors duration-300 ${isCompleted ? 'bg-slate-900/50 opacity-50' : 'bg-transparent'}`}>
                    <div className="p-4">
                      <div className="flex items-center gap-4">
                        <button
                          onClick={(e) => { e.stopPropagation(); onToggleComplete(dayKey, ex.name); }}
                          className={`shrink-0 transition-all duration-200 ${isCompleted ? 'text-green-500 scale-110' : 'text-slate-600 hover:text-slate-400'}`}
                          aria-label={isCompleted ? `Marcar ${ex.name} como no completado` : `Marcar ${ex.name} como completado`}
                        >
                          {isCompleted ? <CheckCircle size={28} fill="rgba(34, 197, 94, 0.2)" /> : <Circle size={28} />}
                        </button>
                        <div className="flex-1 cursor-pointer" onClick={() => setExpandedExercise(isExpanded ? null : ex.name)}>
                          <div className="flex justify-between items-start">
                            <div>
                              <h3 className={`font-bold text-base ${isCompleted ? 'text-slate-500 line-through' : 'text-slate-200'}`}>{ex.name}</h3>
