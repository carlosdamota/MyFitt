...
interface RoutineEditorProps {
  ...
  onCancel: () => void;
  onShare?: (routine: Routine) => Promise<string | null>;
}
...
const RoutineEditor: React.FC<RoutineEditorProps> = ({
  ...
  const handleDragEnd = (event: DragEndEvent, blockIndex: number): void => {
    ...
    if (over && active.id !== over.id) {
      ...
      newBlocks[blockIndex].exercises = arrayMove(
        newBlocks[blockIndex].exercises,
        oldIndex,
        newIndex,
      );
  ...
  const handleChange = (field: keyof Routine, value: string): void => {
    setRoutine((prev) => ({ ...prev, [field]: value }));
  };

  const addBlock = () => {
    const newBlock = {
      id: Date.now(), // ID único basado en tiempo para evitar duplicados al borrar/añadir
      rest: 60,
      exercises: [],
    };
  ...
  const addExercise = (blockIndex: number): void => {
    const newBlocks = [...routine.blocks];
  ...
  const removeExercise = (blockIndex: number, exerciseIndex: number): void => {
    const newBlocks = [...routine.blocks];
    newBlocks[blockIndex].exercises = newBlocks[blockIndex].exercises.filter(
      ...
      <div className='sticky top-0 bg-slate-900/90 backdrop-blur border-b border-slate-800 p-4 flex justify-between items-center z-10'>
        ...
        <div className='flex gap-2'>
          ...
          <button
            ...
            className='bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-blue-500'
          >
          ...
          <div className='max-w-md mx-auto flex flex-col gap-2'>
            ...
            <div className='flex gap-2'>
              ...
              <button
                ...
                className='bg-blue-600 hover:bg-blue-500 text-white px-3 py-2 rounded flex items-center gap-2 transition-colors'
              >
