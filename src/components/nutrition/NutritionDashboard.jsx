...
import { PieChart, Plus, Trash2, Utensils, Flame, Droplet, Wheat, Loader, Sparkles, ChevronLeft, ChevronRight, Calendar, TrendingUp, AlertTriangle } from 'lucide-react';
...
const NutritionDashboard = ({ user }) => {
  ...
  const dayTotals = useMemo(() => getDayTotals(selectedDate), [selectedDate, logs]);
      ...
      <div className="space-y-1">
        <div className="flex justify-between items-center text-xs">
          ...
          <span className={`font-mono ${isOver ? 'text-red-400' : 'text-slate-300'}`}>
            {Math.round(current)}/{target}g {isOver && <AlertTriangle size={10} className="inline text-red-400" />}
          </span>
        </div>
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      {showRateLimitError && (
        <RateLimitError 
          message={rateLimitNutrition.error || "Has alcanzado el límite de 50 logs de nutrición con IA por día"}
      ...
      <div className="bg-slate-900/50 p-3 rounded-2xl border border-slate-800 flex items-center justify-between">
        <button onClick={() => changeDate(-1)} className="p-2 hover:bg-slate-800 rounded-lg transition-colors">
          <ChevronLeft size={18} className="text-slate-400" />
        </button>
        <div className="flex items-center gap-2">
          <Calendar size={16} className="text-blue-400" />
          <span className="text-sm font-bold text-white">
            {isToday ? 'Hoy' : selectedDate.toLocaleDateString('es-ES', { day: 'numeric', month: 'short' })}
          </span>
        </div>
        <button onClick={() => changeDate(1)} disabled={isToday} className="p-2 hover:bg-slate-800 rounded-lg transition-colors disabled:opacity-30">
      ...
      <div className="bg-slate-900/50 p-4 rounded-2xl border border-slate-800">
        <h3 className="text-sm font-bold text-slate-400 uppercase mb-4 flex items-center gap-2"><Utensils size={16} /> Resumen del Día</h3>
        ...
        <div className="flex items-center gap-6 mb-6">
          ...
          <div className="relative w-24 h-24 rounded-full flex items-center justify-center shrink-0" style={chartStyle}>
            <div className="absolute inset-2 bg-slate-950 rounded-full flex flex-col items-center justify-center">
              <span className={`text-xl font-bold ${isCaloriesOver ? 'text-red-400' : 'text-white'}`}>{Math.round(dayTotals.calories)}</span>
          ...
          <div className="flex-1">
            ...
            <div className="flex justify-between text-[10px]">
              <span className={isCaloriesOver ? 'text-red-400 font-bold' : 'text-slate-500'}>
                {isCaloriesOver ? `+${Math.round(dayTotals.calories - targets.calories)} kcal` : `${Math.round(targets.calories - dayTotals.calories)} restantes`}
              </span>
        ...
        <div className="space-y-3">
          <MacroBar label="Proteína" current={dayTotals.protein} target={targets.protein} color="text-blue-400" icon={Droplet} />
          <MacroBar label="Carbohidratos" current={dayTotals.carbs} target={targets.carbs} color="text-purple-400" icon={Wheat} />
          <MacroBar label="Grasas" current={dayTotals.fats} target={targets.fats} color="text-yellow-400" icon={Flame} />
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {loading ? (
           ...
           <p className="text-center text-slate-600 text-xs py-4 italic">No hay comidas registradas este día.</p>
        ) : (
          logs.filter(l => new Date(l.date).toDateString() === selectedDate.toDateString()).map((log) => (
            <div key={log.id} className="bg-slate-900 p-3 rounded-xl border border-slate-800 flex justify-between items-center animate-in slide-in-from-bottom-2">
              ...
              {isToday && <button onClick={() => deleteFoodLog(log.id)} className="text-slate-600 hover:text-red-400 p-2 transition-colors"><Trash2 size={16} /></button>}
            </div>
          ))
        )}
      </div>
