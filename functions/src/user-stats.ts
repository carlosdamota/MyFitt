...
export const createUpdateUserStatsFunction = (params: { appId: string }) => {
  ...
  return onDocumentCreated(
    ...
    async (event) => {
      ...
      console.log(`[updateUserStats] Starting processing for session ${sessionId} of user ${userId}`);
      ...
      try {
        await db.runTransaction(async (transaction) => {
          ...
          const sessionDate = data.date || new Date().toISOString();
        ...
        try {
          ...
          ph.capture({
            ...
            properties: {
              ...
              exercises_count: Object.keys(data.logs || {}).length,
            },
          });
        } catch (phError) {
          console.error("Failed to track stats aggregation in PostHog:", phError);
        }
        console.log(`[updateUserStats] Successfully updated stats for session ${sessionId}`);
        return;
      } catch (error) {
        console.error(`[updateUserStats] Error processing session ${sessionId}:`, error);
        throw error;
      }
    },
  );
